...
# removed "colClasses" = "character" and quote=""  arguments in read.table so that columnnames will not be read in with
# "X" at the beginning.

# Also, made dedupe=FALSE as default.

FormatFoods <- function(input_fn, output_fn, dedupe=FALSE)
{
    fdata <- read.table(input_fn, header = TRUE, sep="\t", strip.white=T)
